...
import { MyLocalStorage } from "../../../shared/services/storage";
...
export const loadCartEffect = createEffect(
    (actions$ = inject(Actions), storage = inject(MyLocalStorage)) => {
        return actions$.pipe(
...
export const persistCartEffect = createEffect(
    (actions$ = inject(Actions), store = inject(Store), storage = inject(MyLocalStorage)) => {
        return actions$.pipe(
            ofType(
